...
export default function Settings() {
  ...
  const [claudeModel, setClaudeModel] = useState<ClaudeModel | "">(
    CLAUDE_MODELS[0],
  );
  const [jiraLabelTrigger, setJiraLabelTrigger] = useState("");
  const [jiraLabelAdd, setJiraLabelAdd] = useState("");
  const [jiraBotAccountId, setJiraBotAccountId] = useState("");
  ...
  if (error) {
    return (
      <ErrorCard
        message={t("settings.errorLoading", {
    ...
    <div className="max-w-xl mx-auto space-y-6">
      ...
      <div className="space-y-6">
          ...
          <div>
            ...
            <p className="text-sm text-dark-500 mt-1 mb-3">
              <Trans
                ...
                components={{
                  name: (
                    <code className="px-1.5 py-0.5 bg-dark-800 rounded text-accent" />
                  ),
                }}
              />
            </p>
            <div
              className={`flex items-center bg-dark-950/50 border rounded-lg transition-all focus-within:ring-2 focus-within:border-transparent ${
                showError
        ...
        <div className="glass-card p-6 space-y-6">
          ...
          <div>
            <label
              htmlFor="jiraBotAccountId"
              className="block text-sm font-medium text-dark-200"
            >
              {t("settings.jira.botAccountId.label")}
            </label>
            <p className="text-sm text-dark-500 mt-1 mb-3">
              {t("settings.jira.botAccountId.description")}
            </p>
            <input
              type="text"
              id="jiraBotAccountId"
              value={jiraBotAccountId}
              onChange={(e) => setJiraBotAccountId(e.target.value.trim())}
              onBlur={() => saveField("jiraBotAccountId", jiraBotAccountId)}
              onKeyDown={(e) => {
                if (e.key === "Enter") (e.target as HTMLInputElement).blur();
              }}
              autoComplete="off"
              ...
              placeholder={t("settings.jira.botAccountId.placeholder")}
            />
            <p className="text-sm text-dark-600 mt-3 italic">
              {t("settings.jira.botAccountId.hint")}
            </p>
          </div>
          ...
          <div>
            <label
              htmlFor="jiraLabelTrigger"
              className="block text-sm font-medium text-dark-200"
            >
              {t("settings.jira.trigger.label")}
            </label>
            <p className="text-sm text-dark-500 mt-1 mb-3">
              {t("settings.jira.trigger.description")}
            </p>
            <input
              type="text"
              id="jiraLabelTrigger"
              ...
              autoComplete="off"
              className="w-full px-4 py-3 bg-dark-950/50 border border-dark-700 rounded-lg text-white placeholder-dark-500 focus:outline-none focus:ring-2 focus:ring-accent focus:border-transparent"
              placeholder={t("settings.jira.trigger.placeholder")}
            />
            <p className="text-sm text-dark-600 mt-3 italic">
              {t("settings.jira.trigger.hint")}
            </p>
          </div>
          ...
          <div>
            <label
              htmlFor="jiraLabelAdd"
              className="block text-sm font-medium text-dark-200"
            >
              {t("settings.jira.add.label")}
            </label>
            <p className="text-sm text-dark-500 mt-1 mb-3">
              {t("settings.jira.add.description")}
            </p>
            <input
              type="text"
              id="jiraLabelAdd"
              ...
              autoComplete="off"
              className="w-full px-4 py-3 bg-dark-950/50 border border-dark-700 rounded-lg text-white placeholder-dark-500 focus:outline-none focus:ring-2 focus:ring-accent focus:border-transparent"
              placeholder={t("settings.jira.add.placeholder")}
            />
            <p className="text-sm text-dark-600 mt-3 italic">
              {t("settings.jira.add.hint")}
            </p>
          </div>
        </div>
